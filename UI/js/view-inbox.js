const feedback = document.querySelector('#feedback');
	const messageBtns = document.querySelectorAll('.messages-nav li');
	...
	const userId = localStorage.epicMailUserId;
...
const createMessage = (messageItem) => {
  const {
 message_id, subject, email, sender_id, message, status, photo 
} = messageItem;
  const id = message_id;
...
const messageBtnHandler = (e) => {
  ...
  if (el.tagName === 'LI' || el.tagName === 'I') {
    ...
    msgHeader.innerText = 			`${newHeader}${category === 'draft' ? 's' : category === 'all' ? ' Received Messages' : ' Messages'}`;
...
const viewMessage = async (e) => {
  try {
    const msg = e.target;
		  const msgId = msg.parentElement.parentElement.getAttribute('data-message-id');
		  const msgType = msg.parentElement.parentElement.getAttribute('data-message-type');
...
const editDraft = (e) => {
  ...
  const subject = draft.parentElement.parentElement.parentElement.previousElementSibling.innerText;
	const message = draft.nextElementSibling.innerText;
	const receiver = draft.getAttribute('data-draft-receiver');
	const id = draft.id;

  const draftMessageData = JSON.stringify({
...
const handleMsgContainerClick = (e) => {
  if (e.target.classList.contains('message-title')) {viewMessage(e);}

  if (e.target.classList.contains('retract-message')) {retractMessage(e);}

  if (e.target.classList.contains('delete-message')) {deleteMessage(e);}

  if (e.target.classList.contains('edit-draft')) {editDraft(e);}
};
