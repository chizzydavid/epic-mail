...
const User = {
  async createUser(req, res) {
    ...
    try {
      const { rows } = await db.query(query, [req.values.email]);
      if (rows[0]) {
        return res.status(400).json({ status: 400, message: 'This email has already been registered.' });
      }
    } catch (e) {
      return res.status(400).json({ status: 500, error: `An error occured while creating your account. ${e}` });
    }
  ...
  async loginUser(req, res) {
    ...
    try {
      ...
      const token = Helper.generateToken(rows[0].user_id);
      return res.status(200).json({ status: 200, data: [{ token, userId: rows[0].user_id }] });
    } catch (e) {
  ...
  async deleteUser(req, res) {
    ...
    try {
      ...
      if (!rowCount) { return res.status(404).json({ status: 404, error: 'User not found.' }); }

      const deleteQuery = 'DELETE FROM users WHERE user_id = $1';
      const { rows } = await db.query(deleteQuery, [req.params.id]);
      if (!rows[0]) { return res.status(200).json({ status: 200, message: 'User successfully deleted.' }); }
    } catch (e) {
